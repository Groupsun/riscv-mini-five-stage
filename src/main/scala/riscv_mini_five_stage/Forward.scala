...
class Forward extends Module with Config {
  ...
  val ex_wb_a_con  = (io.mem_wb_Reg_Write.toBool()        &&
                      io.ex_mem_rd =/= 0.U                &&
                      !((io.ex_mem_Reg_Write.toBool()     &&
                        io.ex_mem_rd =/= 0.U              &&
                        (io.ex_mem_rd === io.id_ex_rs1))) &&
                      (io.mem_wb_rd === io.id_ex_rs1))

  io.Forward_A := Mux(ex_mem_a_con, Forward_A_ex_mem_rd,
                      Mux(ex_wb_a_con, Forward_A_mem_wb_rd, Forward_A_rs1))
  ...
  io.Forward_B := Mux(ex_mem_b_con, Forward_B_ex_mem_rd,
                      Mux(ex_wb_b_con, Forward_B_mem_wb_rd, Forward_B_rs1))
